...
token_t *nil(token_t *op, token_t *par1, token_t *par2)
{
    ...
    if (strcmp(par2->name, "nil") == 0)
    {
        if (strcmp(op->name, "==") == 0)
        {
            printf("MOVE GF@$des bool@true\n");
        }
        else
        {
            printf("MOVE GF@$des bool@false\n");
        }
    }
    else
    {
...
token_t *int_int(token_t *op, token_t *par1, token_t *par2, symtable_t *symtab, bool variant)
{
    ...
    if (variant == 0)
    {
        ...
        if (strcmp(op->name, "/") == 0)
        {
            ...
            printf("JUMPIFNEQ $%s$%llu$notZero %s@%s %s@0\n"
                   "EXIT int@9\n"
    ...
    else
    {
        if (strcmp(op->name, "/") == 0)
        {
            printf("JUMPIFNEQ $%s$%llu$notZero %s@%s %s@0x0p+0\n"
                   "EXIT int@9\n"
                   "LABEL $%s$%llu$notZero\n", symtab->name, label_n, param2, print2, div_type, symtab->name, label_n);
        }
    }

    printf("%s%s GF@$des %s@%s %s@%s\n", div, operator(op->name, 0), param1, print1, param2, print2);

    if (strcmp(op->name, "<=") == 0 || strcmp(op->name, ">=") == 0)
    {
        printf("EQ GF@$tmp %s@%s %s@%s\n"
...
token_t *int_dbl(token_t *op, token_t *par1, token_t *par2, symtable_t *symtab, bool switched)
{
    ...
    printf("INT2FLOAT GF@$tmp %s@%s\n", param1, print1);

    if (switched)
    {
        if (strcmp(op->name, "/") == 0)
        {
            printf("JUMPIFNEQ $%s$%llu$notZero %s@%s int@0\n"
                   "EXIT int@9\n"
                   "LABEL $%s$%llu$notZero\n", symtab->name, label_n, param1, print1, symtab->name, label_n);
        }
        printf("%s GF@$des %s@%s GF@$tmp\n", operator(op->name, 0), param2, print2);
    ...
    else
    {
        if (strcmp(op->name, "/") == 0)
        {
            printf("JUMPIFNEQ $%s$%llu$notZero %s@%s float@0x0p+0\n"
                   "EXIT int@9\n"
                   "LABEL $%s$%llu$notZero\n", symtab->name, label_n, param2, print2, symtab->name, label_n);
        }
        printf("%s GF@$des GF@$tmp %s@%s\n", operator(op->name, 0), param2, print2);
    }
...
token_t *int_id(token_t *op, token_t *par1, token_t *par2, symtable_t *symtab, bool switched)
{
    ...
    if (strcmp(op->name, "==") == 0 || strcmp(op->name, "!=") == 0)
    {
        ...
        if (strcmp(op->name, "!=") == 0)
        {
            printf("NOT GF@$des GF@$des\n");
        }
        printf("LABEL END$%llu\n", label_n);
    }
    else
    {
        printf("TYPE GF@$type %s@%s\n"
...
token_t *gen_expr(token_t *op, token_t *param1, token_t *param2, symtable_t *symtab)
{
    ...
    {
        case INT_INT:
            return int_int(op, param1, param2, symtab, false);
            break;
        case INT_DBL:
            return int_dbl(op, param1, param2, symtab, 0);
            break;
        case INT_STR:
            return int_str(op, param1, param2);
            break;
        case INT_ID:
        ...
        case DBL_INT:
            ...
            break;
        case DBL_DBL:
            return int_int(op, param1, param2, symtab, true);
            break;
        case DBL_STR:
        ...
        case DBL_ID:
            ...
            break;
        case STR_INT:
            return int_str(op, param2, param1);
            break;
        case STR_DBL:
        ...
        /*case NULL_NULL:
            ...
            break;*/
        case ID_NIL:
            return nil(op, param2, param1);
            break;
        case NIL_ID:
            return nil(op, param1, param2);
            break;
...
int main()
{
    ...
    token_t *tmp12 = gen_expr(op,  token, token2, symtable);
