...
token_t *int_id(token_t *op, token_t *par1, token_t *par2, symtable_t *symtab, bool switched, list_t *code_buffer, bool in_stat)
{
    ...
    else
    {
        print_or_append(code_buffer, in_stat, "TYPE GF@$type %s@%s\n"
                        "JUMPIFNEQ $%s$%llu$string GF@$type string@string\n", param2, print2, symtab->name, label_n);
        print_or_append(code_buffer, in_stat, "DEFVAR %s@%s%llu$type\n"
                        "DEFVAR %s@%s%llu$tmp\n"
        ...
        if (switched)
        {
            print_or_append(code_buffer, in_stat, "%s %s@%s %s@%s%llu$tmp %s@$tmp%llu$int\n", operator(op->name, 0),
                            frame,
        ...
        {
            print_or_append(code_buffer, in_stat, "%s %s@%s %s@$tmp%llu$int %s@%s%llu$tmp \n", operator(op->name, 0),
                            frame,
                            des->info.ptr->var.key,
                            frame,
                            label_n,
        ...
        if (strcmp(op->name, "<=") == 0 || strcmp(op->name, ">=") == 0)
        {
            print_or_append(code_buffer, in_stat, "DEFVAR %s@%s$eq\n"
                            "DEFVAR %s@%s$or\n"
                            "MOVE %s@%s$or %s@%s\n"
...
token_t *dbl_id(token_t *op, token_t *par1, token_t *par2, symtable_t *symtab, bool switched, list_t *code_buffer, bool in_stat)
{
    ...
    else
    {
        ...
        if (switched)
        {
            print_or_append(code_buffer, in_stat, "%s %s@%s %s@%s%llu$tmp %s@%s\n", operator(op->name, 0),
                            frame,
                            des->info.ptr->var.key,
                            frame,
                            print2,
        ...
        {
            print_or_append(code_buffer, in_stat, "%s %s@%s %s@%s %s@%s%llu$tmp \n", operator(op->name, 0),
                            frame,
                            des->info.ptr->var.key,
                            param1,
                            print1,
...
token_t *gen_expr(token_t *op, token_t *param1, token_t *param2, symtable_t *symtab, list_t *code_buffer, bool in_stat)
{
    ...
    switch (type(param1, param2))
    {
        ...
        case DBL_DBL:
            ...
            break;
        case DBL_STR:
            return dbl_str(op, param1, param2, symtab, code_buffer, in_stat);
            break;
        case DBL_ID:
        ...
        case STR_INT:
            ...
            break;
        case STR_DBL:
            return dbl_str(op, param2, param1, symtab, code_buffer, in_stat);
            break;
        case STR_STR:
